...
private class DependencyCleanupPanel(private val project: Project) : JPanel(BorderLayout()) {
        ...
        previewTable = JBTable(previewModel).apply {
            ...
            columnModel.getColumn(0).cellEditor = object : DefaultCellEditor(JCheckBox()) {
                override fun getTableCellEditorComponent(
                    ...
                    return (super.getTableCellEditorComponent(table, value, isSelected, row, column) as JCheckBox).apply {
    ...
    private fun setupUI() {
        ...
        if (dependencyType == DependencyType.MAVEN) {
            ...
            val repoPath = mavenService.getLocalRepository()
            controlPanel.add(Box.createVerticalStrut(5))
            controlPanel.add(JLabel("Maven repository: $repoPath").apply {
                alignmentX = Component.LEFT_ALIGNMENT
                font = font.deriveFont(font.size2D - 1f)  // 稍微小一点的字体
                foreground = UIUtil.getContextHelpForeground()  // 使用帮助文本的颜色
            })
        }
        ...
        val previewButton = JButton("Preview Cleanup").apply {  // 修改按钮文字
